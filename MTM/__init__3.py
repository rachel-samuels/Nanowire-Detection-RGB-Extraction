...
def drawBoxesOnRGB(image, tableHit, boxThickness=2, boxColor=(255, 0, 0), showLabel=False, labelColor=(255, 0, 0), labelScale=0.5, showCenter=True, centerColor=(255, 0, 0), centerRadius=3):
    """
    ...
    for _, row in tableHit.iterrows():
        ...
        if showLabel:
            cv2.putText(outImage, text=row['TemplateName'], org=(x, y), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=labelScale, color=labelColor, lineType=cv2.LINE_AA)
        ...
        class ImageKernelMatcher:
            def __init__(self, image_path):
                self.image = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)
                self.kernel_no_rings = 3
                self.kernel_width = x + w
                self.kernel_coord_locs_x = []
                self.kernel_coord_locs_y = []
                self.calculate_kernel_coordinates()

            def calculate_kernel_coordinates(self):
                for col_idx in range(1, self.kernel_no_rings + 1):
                    for row_idx in range(1, self.kernel_no_rings + 1):
                        # Calculation for both X and Y coordinates
                        for cos_multiplier in [-1, 1]:
                            p_centre_x = cos_multiplier * math.cos(math.pi / 6) * col_idx * 2
                            p_centre_y = 2 * math.sin(math.pi / 6) * col_idx - 2 * (self.kernel_no_rings + 1 - row_idx)
                            p_centre_x = int(p_centre_x * self.kernel_width / (
                                        (self.kernel_no_rings - 1) * 4 + 6) + self.kernel_width / 2)
                            p_centre_y = int(p_centre_y * self.kernel_width / (
                                        (self.kernel_no_rings - 1) * 4 + 6) + self.kernel_width / 2)

                            if cos_multiplier == -1:
                                self.kernel_coord_locs_x.append((p_centre_x, p_centre_y))
                            else:
                                self.kernel_coord_locs_y.append((p_centre_x, p_centre_y))

            def match_kernel(self, threshold_value):
                match_array = np.zeros((len(self.kernel_coord_locs_x),), dtype=int)

                for i, (x, y) in enumerate(self.kernel_coord_locs_x):
                    if 0 <= y < self.image.shape[0] and 0 <= x < self.image.shape[
                        1]:  # Check if coordinates are within the image bounds
                        if self.image[y, x] > threshold_value:
                            match_array[i] = 1

                return match_array

        # Example usage
        image_path = outImage[y:y+h, x:x+w]
        matcher = ImageKernelMatcher(image_path)
        threshold_value = 200  # Set your threshold value here
        match_result = matcher.match_kernel(threshold_value)
        print(match_result)
