...
def drawBoxesOnRGB(image, tableHit, boxThickness=2, boxColor=(255, 0, 0), showLabel=False, labelColor=(255, 0, 0), labelScale=0.5, showCenter=True, centerColor=(255, 0, 0), centerRadius=3):
    ...
    for _, row in tableHit.iterrows():
        ...
        class KernelMatcher:
            ...
            def calculate_kernel_coordinates(self):
                for col_idx in range(1, self.kernel_no_rings + 1):
                    for row_idx in range(1, self.kernel_no_rings + 1):
                        # Calculate coordinates for both X and Y
                        for cos_multiplier in [-1, 1]:  # To handle both X and Y coordinates
                            ...
                            p_centre_x = p_centre_x * self.kernel_width / (
                                    (self.kernel_no_rings - 1) * 4 + 6) + self.kernel_width / 2
                            p_centre_y = p_centre_y * self.kernel_width / (
                                    (self.kernel_no_rings - 1) * 4 + 6) + self.kernel_width / 2
            ...
            def match_with_region(self, image_region):
                # Assuming image_region is a 2D array of 0s and 1s
                match_array = [0] * 9  # Initialize the match array with 0s
                # Add your logic here to check for a match between the kernel and the image region
                # Update match_array based on the match criteria

                return match_array

        matcher = KernelMatcher()
        image_region = outImage[y:y+h, x:x+w]
        match_result = matcher.match_with_region(image_region)
        print(match_result)
