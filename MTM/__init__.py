...
def computeScoreMap(template, image, method=cv2.TM_CCOEFF_NORMED, mask=None):
    ...
    if mask is not None:
        ...
        if method not in (0, 3):
            mask = None
            warnings.warn(
                "Template matching method not compatible with use of mask (only 0/TM_SQDIFF or 3/TM_CCORR_NORMED).\n-> Ignoring mask.")
            ...
            if not (sameDimension and sameType):
                mask = None
                warnings.warn(
                    "Mask does not have the same dimension or bit depth than the template.\n-> Ignoring mask.")
...
def findMatches(listTemplates, image, method=cv2.TM_CCOEFF_NORMED, N_object=float("inf"), score_threshold=0.5,
    ...
    for index, tempTuple in enumerate(listTemplates):
        ...
        if not isinstance(tempTuple, tuple) or len(tempTuple) < 2:
            raise ValueError(
                "listTemplates should be a list of tuples as ('name','array') or ('name', 'array', 'mask')")
        ...
        templateSmallerThanImage = all(templateDim <= imageDim for templateDim, imageDim in zip(tempImage.shape,
                                                                                                image.shape))  # check both width/height smaller than image or search region

        if not templateSmallerThanImage:
            fitIn = "searchBox" if (searchBox is not None) else "image"
            raise ValueError(
                "Template '{}' at index {} in the list of templates is larger than {}.".format(tempName, index, fitIn))
    ...
    with ThreadPoolExecutor(max_workers=round(os.cpu_count() * .5)) as executor:
        futures = [
            executor.submit(_multi_compute, tempTuple, image, method, N_object, score_threshold, xOffset, yOffset,
                            listHit) for tempTuple in listTemplates]
...
def _multi_compute(tempTuple, image, method, N_object, score_threshold, xOffset, yOffset, listHit):
    ...
    if len(tempTuple) >= 3:  # ie a mask is also provided
        if method in (0, 3):
            ...
            warnings.warn(
                "Template matching method not supporting the use of Mask. Use 0/TM_SQDIFF or 3/TM_CCORR_NORMED.")
    ...
    for peak in peaks:
        ...
        listHit.append(
            {'TemplateName': templateName, 'BBox': (int(peak[1]) + xOffset, int(peak[0]) + yOffset, width, height),
             'Score': corrMap[tuple(peak)]})  # empty df with correct column header


def matchTemplates(listTemplates, image, method=cv2.TM_CCOEFF_NORMED, N_object=float("inf"), score_threshold=0.5,
                   maxOverlap=0.25, searchBox=None):
    """
...
def drawBoxesOnRGB(image, tableHit, boxThickness=2, boxColor=(255, 0, 0), showLabel=False, labelColor=(255, 0, 0),
                   labelScale=0.5, showCenter=True, centerColor=(255, 0, 0), centerRadius=3):
    """
    ...
    The center of the hit can also be displayed with showCenter=True.

    Parameters
    ----------
    - image  : image in which the search was performed

    - tableHit: list of hits as returned by matchTemplates or findMatches

    - boxThickness: int, thickness of bounding box contour in pixels

    - boxColor: (int, int, int), RGB color for the bounding box

    - showLabel: Boolean, display label of the bounding box (field TemplateName)

    - labelColor: (int, int, int), RGB color for the label

    - showCenter: Boolean, display the center of the bounding box

    - centerColor: (int, int, int), RGB color for the center point

    - centerRadius: int, radius of the center point circle in pixels

    Returns
    -------
    outImage: RGB image, original image with predicted template locations depicted as bounding boxes
    """
    if image.ndim == 2:
        outImage = cv2.cvtColor(image, cv2.COLOR_GRAY2RGB)
    else:
        outImage = image.copy()
    ...
    for _, row in tableHit.iterrows():
        ...
        if showCenter:
            ...
            circle_radius=5
            # Draw circles at the top-left and bottom-right corners of the bounding box
            cv2.circle(outImage, (x, y), circle_radius, color=circle_color, thickness=-1)
            cv2.circle(outImage, (x+w, y+h), circle_radius, color=circle_color, thickness=-1)
            #
...
def drawBoxesOnGray(image, tableHit, boxThickness=2, boxColor=255, showLabel=False, labelColor=255, labelScale=0.5):
    ...
    if image.ndim == 3:
        outImage = cv2.cvtColor(image,
                                cv2.COLOR_RGB2GRAY)  # convert to RGB to be able to show detections as color box on grayscale image
    else:
        outImage = image.copy()
    ...
    for _, row in tableHit.iterrows():
        x, y, w, h = row['BBox']
        cv2.rectangle(outImage, (x, y), (x + w, y + h), color=boxColor, thickness=boxThickness)
        if showLabel: cv2.putText(outImage, text=row['TemplateName'], org=(x, y), fontFace=cv2.FONT_HERSHEY_SIMPLEX,
                                  fontScale=labelScale, color=labelColor, lineType=cv2.LINE_AA)
